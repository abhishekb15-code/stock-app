...
export default function Dashboard() {
  ...
  const pnlColor = summary.totalPnl >= 0 ? 'var(--green)' : 'var(--red)';
  const dailyColor = summary.dailyPnl >= 0 ? 'var(--green)' : 'var(--red)';
  ...
  return (
    <div className="fade-in">
      ...
      <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', marginBottom: 24 }}>
        ...
        <MetricCard label="Total P&L" value={signedMoney(summary.totalPnl)} sub={`${summary.totalPnlPercent >= 0 ? '+' : ''}${summary.totalPnlPercent}% all time`} color={pnlColor} />
        <MetricCard label="Today's P&L" value={signedMoney(summary.dailyPnl)} sub="Market hours" color={dailyColor} />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 20, marginBottom: 24 }}>
        ...
        <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
          <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ fontWeight: 600, fontSize: 14 }}>Holdings</span>
            <button className="btn btn-ghost" style={{ fontSize: 11, padding: '5px 10px' }} onClick={() => navigate('/portfolio')}>View All</button>
          </div>
          <table className="data-table">
            <thead>
              <tr>
                <th>Ticker</th><th>Buy</th><th>Current</th><th>P&L</th><th>RSI</th><th>Signal</th>
              </tr>
            </thead>
            <tbody>
              {holdings.map(h => {
                ...
                return (
                  <tr key={h.id} style={{ cursor: 'pointer' }} onClick={() => navigate(`/stock/${h.ticker}`)}>
                    ...
                    <td className="mono">{money(h.currentPrice)}</td>
                    <td className={h.pnl >= 0 ? 'pos mono' : 'neg mono'}>
                      {signedMoney(h.pnl)}
                    </td>
        ...
        <div className="card">
          ...
          <ResponsiveContainer width="100%" height={220}>
            <RechartsPie>
              ...
              <Tooltip formatter={(v) => [money(v), 'Value']} contentStyle={{ background: 'var(--bg-600)', border: '1px solid var(--border)', borderRadius: 8, color: 'var(--text-primary)' }} />
